// Guitar Hero controller XInput BLE example for Pico W
// ...existing code from previous guitar main.c, now in this file...
...
typedef enum { INPUT_BUTTON, INPUT_HAT } input_type_t;
typedef struct { int gpio; input_type_t type; uint16_t input; } gpio_xbox_map_t;
static const gpio_xbox_map_t gpio_input_map[] = {
    ...
    {6,  INPUT_HAT, XBOX_HAT_UP}, {9,  INPUT_HAT, XBOX_HAT_RIGHT},
    {7,  INPUT_HAT, XBOX_HAT_DOWN}, {8,  INPUT_HAT, XBOX_HAT_LEFT},
};
...
static XboxReport report = {
    .buttons = 0, .hat = XBOX_HAT_NOTHING,
    .left_stick_x = XBOX_AXIS_CENTER, .left_stick_y = XBOX_AXIS_CENTER,
    .right_stick_x = XBOX_AXIS_CENTER, .right_stick_y = XBOX_AXIS_CENTER,
...
static uint16_t read_whammy_filtered(void) {
    uint32_t sum = 0;
    for (int sample = 0; sample < WHAMMY_SAMPLES; sample++) sum += adc_read();
...
static void button_poll_timer_handler(btstack_timer_source_t *ts) {
    ...
    for (int i = 0; i < gpio_input_count; i++) {
        ...
        if (gpio_get(m->gpio)) continue;
        if (m->type == INPUT_BUTTON) report.buttons |= m->input;
        else {
            switch (m->input) {
    ...
    if (++whammy_div >= WHAMMY_UPDATE_DIV) {
        ...
        if (whammy_delta >= WHAMMY_DEADBAND) report.right_stick_x = whammy_smoothed;
    }
