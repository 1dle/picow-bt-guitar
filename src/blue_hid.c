...
//write target mac address to the flash
void flash_write_device_address(bd_addr_t p_addr){
    write_flash(p_addr, BD_ADDR_LEN);
}

void flash_get_device_address(uint8_t *p_addr){
    for(int i = 0; i < BD_ADDR_LEN; i++)
        p_addr[i]=read_flash(i);
}
                ...
                case HCI_EVENT_HID_META:
                    switch (hci_event_hid_meta_get_subevent_code(packet)){
                        case HID_SUBEVENT_CONNECTION_OPENED:
                            ...
                            flash_write_device_address(device_addr);
                            //TESTING
...
int btstack_hid(void){
    ...
    if(!connected){
        ...
        flash_get_device_address(device_addr);
